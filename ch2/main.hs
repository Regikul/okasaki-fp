...
insert x t
	= insert' x t `check` t
		where
			...
			insert' n (Node l v r)
				| n < v 
					= insert' n l >>= \l' -> Just $ Node l' v r
				| n > v 
					= insert' n r >>= \r' -> Just $ Node l v r'
				| otherwise 
					= Nothing
			insert' n Empty
				= Just $ Node Empty n Empty
-- ex. 2.4
-- Can't catch exceptions within the pure code. So use Maybe monad.
insert2 :: Ord a => TreeInsert a
insert2 x t
	= insert' x Nothing t `check` t
		where
			check (Just v) _
				= v
			check Nothing v
			...
			insert' n m (Node l v r)
				| n <= v 
					= insert' n (Just v) l >>= \l' -> Just $ Node l' v r
				| otherwise 
					= insert' n m r >>= \r' -> Just $ Node l v r'
			insert' n Nothing Empty
				= empty n
